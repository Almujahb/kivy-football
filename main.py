...
class Goalie(Widget):  # Enemy goalie = Red, Right side
    def __init__(self, start_x, **kwargs):
        super().__init__(**kwargs)
        self.size = (70, 100)  # Swapped: tall now
        self.direction = 1
...
class GoalieLeft(Goalie):  # Your goalie = Blue, Left side
    pass


class MyDefender(Widget):  # Your team = Blue, Left half
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.size = (60, 60)
...
class EnemyDefender(Widget):  # Enemy = Red, Right half
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.size = (60, 60)
        self.speed_x = randint(-1, 1)  # <- SLOWER
...
class Game(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        self.goalie_right = Goalie(start_x=right_goal_line_x - goalie_into_box - 70,
